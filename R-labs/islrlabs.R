...
library(glmnet)
xtr <- data.matrix(Smarket[train,-c(1,7,9)])
ytr <- Smarket[train,9]
xts <- data.matrix(Smarket[!train,-c(1,7,9)])
yts <- Smarket[!train,9]
lasso.cv <- cv.glmnet(x=xtr, y=ytr, family="binomial",alpha=1)
plot(lasso.cv)
names(lasso.cv)
#lasso.fit <- glmnet(x=xtr, y=ytr, family="binomial", alpha=1, lambda=lasso.cv$lambda.1se)
...
y.test=y[test]
mean((pcr.pred-y.test)^2, na.rm=TRUE)
